...
For each iteration n in [start..end]:
  1. If n == 1: target_1(parent) = max_a (reward_a + 0) = act_max_reward, computed from
     children (no bootstrap).
  2. Else: load V_{n-1} (joblib), predict V_{n-1}(child) on every (valid) child, then
     target_n(parent) = max_a [ reward_a + gamma_a * clip_max(V_{n-1}(child_a), 0) ]
     over is_valid==1 actions in both child caches (D1 + D2).
...
def _load_child_cache(cache_dir: Path):
    """Return (features (N,224) mmap, parent_ids (P,), offsets (P+1,), order (N,),
...
def _bellman_max_per_parent(
    ...
    """
    For every parent in this cache, compute max over its valid children of:
        reward + discount * V(child)  (V already <= 0 enforced)
    and write into parent_target at the index implied by parent_id_to_pos_*.

    parent_id_to_pos_offset/count map from this cache's parent_id -> (start, n) inside
    the GLOBAL parent_target array. We are *adding* candidates to whatever's already
    there: the caller starts parent_target = -inf and we max-update.

    cache keys: order, parent_ids, offsets, rewards, discounts, is_valid.
    child_v: prediction for each child row already in original (cache row) order, shape
    (N_rows,).  Note: cache["order"] sorts the rows by parent_id; we must apply it to
    rewards/discounts/is_valid/child_v before we can slice with cache["offsets"].
    """
    order = cache["order"]
    ...
    # candidate value for every child row (already sorted by parent)
    candidates = rewards_sorted + disc_sorted * cv_sorted
    # mark invalid children with -inf so they can't win the max
    candidates[~is_valid_sorted] = -np.inf
    ...
    for i, pid in enumerate(parent_ids):
        ...
        if slot is None:
            continue
        seg_max = float(np.max(candidates[s:e]))
        # max-update parent_target[slot]
...
def main() -> None:
    ...
    def _compute_targets_bellman(v_child_fn) -> np.ndarray:
        ...
        _bellman_max_per_parent(cache_d1, cv1, parent_pos_map_d1, parent_count_d1, parent_target)
        del cv1
        ...
        _bellman_max_per_parent(cache_d2, cv2, parent_pos_map_d2, parent_count_d2, parent_target)
        del cv2
