"""
Build the v6 state-local feature schema = v5 (244d) + rescue-scalar block (4d).

v6 adds 4 single-scalar rescue indicators so HGB can express "a launch budget
within the search space avoids all post-tick SLO violations" without ANDing
together multiple per-B one-hots.

Outlier inspection on v4 (best L31 a=2 had 17 eval rows with abs_err > 0.5)
showed the same pattern in every outlier: y_true=0 but model predicts ~-2,
because the v4 features cannot tell "this state is on the cliff but a 512-
token chunked-prefill rescue exists" apart from "no rescue". The lookahead
block per-B violation counts gives partial signal but HGB still has to AND
across (B=128 viols > 0) AND (B=512 viols == 0).

The rescue scalars resolve that:
  - g_rescue_min_B_pref_viol_zero_norm  = (smallest B with n_post_pref_viol == 0) / 2048
                                         or 1.0 if no B in {128,256,512,1024} achieves it.
  - g_rescue_min_B_dec_viol_zero_norm   = same for decode violations.
  - g_rescue_min_B_total_viol_zero_norm = same for sum of pref+dec violations.
  - g_rescue_pref_viol_drop_with_b_norm = (max_pref_viol(B=128) - min_pref_viol(B>=512)) / 20
                                          (Positive if increasing B reduces pref violations -> rescue exists.)

Total D = 21 (v4 globals) + 24 (lookahead+rescue) + 7*25 (prefill slots) + 7*4 (decode) = 248.
"""
...
RESCUE_B_NORM_DEN: float = 2048.0
D_RESCUE: int = 4
...
D_GLOBAL = D_GLOBAL_V4 + D_LOOKAHEAD + D_RESCUE  # 45
D_TOTAL = D_GLOBAL + N_PREFILL_SLOTS * D_PREFILL_PER_SLOT + N_DECODE_SLOTS * D_DECODE_PER_SLOT  # 248
...
def _compute_lookahead_and_rescue_block(
    ...
    """Compute the 20-d lookahead block and the 4-d rescue block."""
    ...
    look: list[float] = []
    pref_viols_per_B: list[int] = []
    dec_viols_per_B: list[int] = []
    for B in LOOKAHEAD_BUDGETS:
        ...
        pref_viols_per_B.append(n_post_tick_pref_viol)
        dec_viols_per_B.append(n_post_tick_dec_viol)

        look.append(_norm01(n_picked, MAX_REQUESTS_PER_LAUNCH_WINDOW))
        look.append(_norm01(tokens_for_tick, MAX_PREFILL_TOKENS_PER_LAUNCH_WINDOW))
        look.append(_norm01(tick_time, TICK_TIME_NORM_DEN))
        look.append(_norm01(n_post_tick_pref_viol, LOOKAHEAD_PREFILL_VIOL_DEN))
        look.append(_norm01(n_post_tick_dec_viol, LOOKAHEAD_DECODE_VIOL_DEN))

    # Rescue scalars
    def _min_B_with_zero(viols: list[int]) -> float:
        for i, B in enumerate(LOOKAHEAD_BUDGETS):
            if viols[i] == 0:
                return float(B)
        return float(LOOKAHEAD_BUDGETS[-1] * 2)

    min_B_pref = _min_B_with_zero(pref_viols_per_B)
    min_B_dec = _min_B_with_zero(dec_viols_per_B)
    min_B_total = _min_B_with_zero([p + d for p, d in zip(pref_viols_per_B, dec_viols_per_B)])

    # Drop in pref violations from B=128 to B>=512
    if pref_viols_per_B:
        viol_at_smallest = pref_viols_per_B[0]
        if len(pref_viols_per_B) >= 3:
            viol_at_largest = min(pref_viols_per_B[2], pref_viols_per_B[3]) if len(pref_viols_per_B) >= 4 else pref_viols_per_B[2]
        else:
            viol_at_largest = pref_viols_per_B[-1]
        drop = max(0, viol_at_smallest - viol_at_largest)
    else:
        drop = 0

    rescue: list[float] = [
        _norm01(min_B_pref, RESCUE_B_NORM_DEN),
        _norm01(min_B_dec, RESCUE_B_NORM_DEN),
        _norm01(min_B_total, RESCUE_B_NORM_DEN),
        _norm01(drop, LOOKAHEAD_PREFILL_VIOL_DEN),
    ]
...
def extract_features_one_record(record: dict) -> np.ndarray:
    ...
    out[D_GLOBAL_V4:D_GLOBAL_V4 + D_LOOKAHEAD] = np.asarray(lookahead_block, dtype=np.float32)
    out[D_GLOBAL_V4 + D_LOOKAHEAD:D_GLOBAL] = np.asarray(rescue_block, dtype=np.float32)
...
def feature_names() -> list[str]:
    ...
    names.extend([
        "g_rescue_min_B_pref_viol_zero_norm",
        "g_rescue_min_B_dec_viol_zero_norm",
        "g_rescue_min_B_total_viol_zero_norm",
        "g_rescue_pref_viol_drop_with_b_norm",
    ])
