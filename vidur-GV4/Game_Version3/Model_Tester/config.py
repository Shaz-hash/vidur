...
class ModelTesterConfig:
    ...
    # If True, skip the model-vs-model cycle2 and only run model_adv_vs_trivial_ctrl.
    # Useful for SJF-budget sweeps where cycle2 is invariant of the trivial budget.
    skip_model_ctrl_cycle: bool = False

    # If True, skip the trivial-controller baseline and only run model-vs-model.
    # This is the self-play mode used for AlphaGoZero replay generation.
    only_model_ctrl_cycle: bool = False
    ...
    # Parallel arena execution. arena_num_processes > 1 fans games out across
    # spawn-method worker processes; each worker rebuilds its own simulator
    # bundle. arena_worker_threads bounds BLAS/OpenMP pools inside each
    # worker so they don't oversubscribe a multi-process run.
    arena_num_processes: int = 1
    arena_worker_threads: int = 1
    arena_mp_start_method: str = "spawn"
    arena_maxtasks_per_child: int = 0  # 0 -> no recycling
