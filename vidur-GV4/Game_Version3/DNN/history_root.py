...
class HistoryRootGenerator:
    ...
    def _decision_time_before_action(
        self,
        ...
        try:
            ...
            has_pending_tick = bool(self.env._v2_has_pending_adv_tick(state))
        except Exception:
            return float(start_time)

        reqs = getattr(action, "requests", None) or []
        stop_ids = getattr(action, "stop_decode_ids", None) or []
        is_strict_noop = (not reqs) and (not stop_ids)

        if is_strict_noop and (not has_pending_tick) and (float(start_time) + 1e-12 < tick):
            return float(start_time)
        return float(tick)
    ...
    def _log_step(
        ...
        *,
        ...
        if decision_state_time is None:
            decision_state_time = self._decision_time_before_action(
                state_after,
                str(player_acted),
                action,
                float(start_time),
        ...
        for _ilog in active_iter_loggers:
            ...
            _ilog.log_expand(
                ...
                stage_total_time=max(0.0, float(end_time) - float(start_time)),
                prior=1.0,
                reward=0.0,
                nn_called=False,
    ...
    def _log_history_trace_step(
        ...
        *,
        ...
        self._log_step(
            ...
            decision_state_time=decision_state_time,
    ...
    def _advance_to_branching_with_context(
        ...
        *,
        ...
        while forced < int(max_forced_steps):
            ...
            decision_state_time = self._decision_time_before_action(state, acted, action, start_time)
            ...
            depth = int(depth) + 1
            forced += 1
            end_time = float(getattr(state.simulator, "_time", start_time))
    ...
    def _expand_one_child(
        ...
        *,
        ...
        start_time = float(getattr(parent.simulator, "_time", 0.0))
        decision_state_time = self._decision_time_before_action(parent, node.player, action, start_time)
        ...
        end_time = float(getattr(child.simulator, "_time", start_time))
        log_node_id = self._log_history_trace_step(
            session=session,
