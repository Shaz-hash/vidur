...
def _prepare_root() -> tuple[Any, Any, Any, Any, Any, int, Any, dict[str, Any], Any]:
    from vidur.Game_Version3.tests import native_logger_tests as nlt
    ...
    if selected is None:
        raise RuntimeError("no multi-action root found")

    return native, args, cfg_python, env, runner, int(args.history_seed), selected, payload, simulator
...
def _run_native(
    *,
    ...
    out = native.search_mcts_dnn_torchscript(
        runtime,
        0,
...
def main() -> None:
    native, args, cfg_python, env, runner, seed, selected, payload, _simulator = _prepare_root()
    pr, root_state, root_player, root_depth, valid_count = selected
    print(
        f"root_id={int(pr.root_id)} player={root_player} depth={root_depth} "
