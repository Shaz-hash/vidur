"""Coordinate multi-server GV3 adversary-root generation jobs.

This script is intentionally a thin SSH/rsync coordinator. Each remote machine
runs rootGeneration.py locally, using its own multiprocessing workers. The
adversary dataset keeps only adversary roots with more than one canonical action.
"""
...
EXPERIMENT_NAME = "bellman_v4_adv_300k_adversary_roots_hops0_750_min2canon"
DEFAULT_REMOTE_REPO = "/home/ubuntu/vidur-classical-search"
DEFAULT_LOCAL_REPO = Path(__file__).resolve().parents[2]
DEFAULT_ROOTS_PER_SERVER = 60_000
DEFAULT_TOTAL_ROOTS = 300_000
DEFAULT_MIN_SELECTED_RATIO = 0.0
DEFAULT_TARGET_ABS_THRESHOLD = 1e99
...
DEFAULT_MAX_CANDIDATE_MULTIPLIER = 100
DEFAULT_SEED = 2027
DEFAULT_MIN_CANONICAL_ACTIONS = 2
...
class JobSpec:
    ...
    min_canonical_actions: int = DEFAULT_MIN_CANONICAL_ACTIONS
...
def parse_hop_intervals(raw: str | None, *, expected_parts: int) -> list[tuple[int, int]] | None:
    ...
    for part in str(raw).split(","):
        ...
        if not part:
            continue
        if "-" not in part:
            raise ValueError(f"invalid hop interval {part!r}; expected START-END")
        start_s, end_s = part.split("-", 1)
        start = int(start_s.strip())
        end = int(end_s.strip())
        if end < start:
            raise ValueError(f"invalid hop interval {part!r}; end must be >= start")
        intervals.append((start, end))
    if len(intervals) != int(expected_parts):
        raise ValueError(
            f"--hop-intervals produced {len(intervals)} intervals, expected {int(expected_parts)}"
        )
    return intervals
...
def build_jobs(args: argparse.Namespace, *, smoke: bool = False) -> list[JobSpec]:
    ...
    intervals = parse_hop_intervals(getattr(args, "hop_intervals", None), expected_parts=len(hosts))
...
def root_generation_command(job: JobSpec) -> list[str]:
    ...
    if bool(job.include_history_trace_logs):
        cmd.append("--include-history-trace-logs")
    return cmd
...
def sync_host(host: str, remote_repo: str) -> None:
    ...
    compile_cmd = (
        ...
        "vidur/bellman_v4_adv_2000k_multiprocess/multi_server_adv_root_generation.py"
    )
...
def add_common_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--hop-intervals",
        default=None,
        help="Comma-separated explicit inclusive hop intervals, e.g. 0-100,100-200.",
    )
    ...
    parser.add_argument(
        "--min-canonical-actions",
        type=int,
        default=DEFAULT_MIN_CANONICAL_ACTIONS,
        help="Keep adversary roots with at least this many canonical actions.",
    )
...
def parse_args() -> argparse.Namespace:
    ...
    for name in ("sync", "launch", "status", "tail", "stop", "collect", "smoke"):
        ...
        if name == "collect":
            p.add_argument("--local-collect-dir", default=str(_repo_root() / "simulator_output/GV3_Agent/ModelSearchBed/collected_300k_adversary_roots"))
        if name == "smoke":
