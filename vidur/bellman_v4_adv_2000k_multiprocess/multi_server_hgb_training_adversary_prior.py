#!/usr/bin/env python3
"""Train a GV3 adversary policy/prior HGB head from native MCTS visit targets.
...
3. assemble-local on the training host, default worker 5:
   join 226D state features + 7D adversary action features + centered-logit target.
4. prepare-split-local on the training host:
   select 30k random roots for eval, rest for train.
...
where the mean is over canonical actions from the same adversary root.
"""
...
DEFAULT_TRAIN_HOST = "bellman-classical-worker-5"
DEFAULT_TARGET_MODEL_LABEL = "worker1_200k_alpha2__hgb_sq_63leaf_1050iter_a2__v47__"
DEFAULT_MCTS_ITERATIONS = 10_000
DEFAULT_TARGET_DIR_SUFFIX = "adversary_effectcanon_full"
DEFAULT_ACTION_FEATURE_VERSION = "adversary_action_features_effectcanon_v1"
DEFAULT_POLICY_ALPHA = 1.0
DEFAULT_SEED = 20260617
DEFAULT_EVAL_ROOTS = 30_000
DEFAULT_THREADS = 48
...
DEFAULT_STATE_FEATURE_HOST = "bellman-classical-worker-5"
DEFAULT_POLICY_WORK_DIR = "{base}/{experiment_name}/adversary_prior_training_effectcanon"
DEFAULT_TARGET_BASE = (
...
DEFAULT_ACTION_FEATURE_BASE = (
    "{base}/{experiment_name}/adversary_prior_action_features/{action_feature_version}"
)
...
FALLBACK_ACTION_FEATURE_NAMES = [
    ...
    "a_adv_stop_rule__stop_none",
    "a_adv_stop_rule__stop_longest_decode",
    "a_adv_stop_rule__stop_shortest_decode",
    "a_adv_stop_rule__stop_all_decodes_over_512",
    "a_adv_stop_rule__stop_all_decodes_over_216",
]
...
def _action_feature_names() -> list[str]:
    try:
        from vidur.bellman_v4_adv_2000k_multiprocess.multi_server_hgb_adversary_prior_feature_building import ACTION_FEATURE_NAMES
...
def _load_raw_targets(input_dir: Path) -> list[dict[str, Any]]:
    ...
    for path in _target_csv_files(input_dir):
        for row in _read_csv_rows(path):
            if str(row.get("player", "adversary")) != "adversary":
                continue
        ...
        summary_rows.append(
            {
                ...
                "player": first.get("player", "adversary"),
                "root_player": first.get("root_player", "adversary"),
...
def _script_path(remote_repo: str) -> str:
    return f"{remote_repo.rstrip('/')}/vidur/bellman_v4_adv_2000k_multiprocess/multi_server_hgb_training_adversary_prior.py"
...
def command_show_plan(args: argparse.Namespace) -> None:
    ...
    print(f"state_feature_host={args.state_feature_host}")
...
def command_sync_state_features(args: argparse.Namespace) -> None:
    src = _state_feature_assembly_dir(args).rstrip("/") + "/"
    dest = _state_feature_assembly_dir(args).rstrip("/") + "/"
    _ssh(args.train_host, f"mkdir -p {shlex.quote(dest)}")

    if args.state_feature_host == args.train_host:
        remote = (
            "set -e; "
            f"rsync -az {shlex.quote(src)} {shlex.quote(dest)}; "
            f"echo synced state_features src={shlex.quote(src)} dest={shlex.quote(dest)} mode=local_on_train_host"
        )
        res = _ssh(args.train_host, remote)
        print(res.stdout, end="" if res.stdout.endswith("\n") else "\n")
        return

    tmp_root = Path(tempfile.mkdtemp(prefix="adversary_prior_state_features_sync_"))
    try:
        _rsync(f"{args.state_feature_host}:{src}", str(tmp_root) + "/")
        _rsync(str(tmp_root) + "/", f"{args.train_host}:{dest}")
        print(
            f"synced state_features src={args.state_feature_host}:{src} dest={args.train_host}:{dest} mode=local_relay",
            flush=True,
        )
    finally:
...
def command_sync_inputs(args: argparse.Namespace) -> None:
    ...
    for worker in _parse_workers(args.workers):
        ...
        tmp_root = Path(tempfile.mkdtemp(prefix=f"adversary_prior_sync_{worker.worker_id}_"))
        try:
...
def add_common(p: argparse.ArgumentParser) -> None:
    ...
    p.add_argument("--workers", default="worker5,worker6,worker7,worker8")
    ...
    p.add_argument("--state-feature-host", default=DEFAULT_STATE_FEATURE_HOST)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser("sync-state-features")
    add_common(p)
    p.set_defaults(func=command_sync_state_features)
