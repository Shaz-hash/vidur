i"""Coordinate per-server child feature building for distributed GV3 child caches."""
...
def build_jobs(args: argparse.Namespace) -> list[JobSpec]:
    ...
    for idx, (host, (hmin, hmax)) in enumerate(zip(hosts, split_hop_intervals(args.hop_min, args.hop_max, len(hosts)))):
        ...
        child_dir = str(args.child_cache_dir).format(root_dataset_dir=root_dir, suffix=suffix, host=host, job_id=f"server_{idx:02d}")
        feat_dir = str(args.feature_dir).format(root_dataset_dir=root_dir, child_cache_dir=child_dir, suffix=suffix, host=host, job_id=f"server_{idx:02d}")
        jobs.append(JobSpec(f"server_{idx:02d}", host, args.remote_repo, root_dir, child_dir, feat_dir, args.num_processes, args.max_in_flight, args.flush_every_shards))
    return jobs

def command_for(job: JobSpec) -> list[str]:
    cmd = [
        f"{job.remote_repo.rstrip('/')}/.venv/bin/python3", "-m", "vidur.bellman_v4_adv.build_child_features_v4Adv",
        "--child-cache-dir", job.child_cache_dir,
        "--out-dir", job.feature_dir,
        "--num-processes", str(job.num_processes),
        "--flush-every-shards", str(job.flush_every_shards),
    ]
    if int(job.max_in_flight) > 0:
        cmd.extend(["--max-in-flight", str(job.max_in_flight)])
    return cmd
...
def launch_job(job: JobSpec) -> None:
    ...
    remote = (
        ...
        f"nohup {cmd} > {out_dir}/child_feature_build.log 2>&1 < /dev/null & "
        f"echo $! > {out_dir}/child_feature_build.pid; echo launched pid=$(cat {out_dir}/child_feature_build.pid) out={out_dir}"
    )
...
def status_job(job: JobSpec, tail_lines: int) -> dict[str, object]:
    out_dir = shlex.quote(job.feature_dir)
    remote = (
        f"OUT={out_dir}; PID=$(cat \"$OUT/child_feature_build.pid\" 2>/dev/null || true); "
        "RUNNING=0; if [ -n \"$PID\" ] && kill -0 \"$PID\" 2>/dev/null; then RUNNING=1; fi; "
        "if [ -f \"$OUT/child_features.summary.json\" ]; then "
        f"{shlex.quote(job.remote_repo.rstrip('/') + '/.venv/bin/python3')} - <<'PY' \"$OUT/child_features.summary.json\"\n"
        "import json,sys\nd=json.load(open(sys.argv[1])); print('SUMMARY_ROWS=%s' % d.get('num_rows','')); print('SUMMARY_DIM=%s' % d.get('feature_dim','')); print('SUMMARY_PARENTS=%s' % d.get('num_parents',''))\nPY\n"
        "fi; echo STATUS_PID=$PID; echo STATUS_RUNNING=$RUNNING; echo STATUS_OUT=$OUT; echo STATUS_LOG_TAIL_BEGIN; "
        f"tail -n {int(tail_lines)} \"$OUT/child_feature_build.log\" 2>/dev/null || true; echo STATUS_LOG_TAIL_END"
    )
    ...
    for line in res.stdout.splitlines():
        ...
        elif line.startswith("SUMMARY_PARENTS="): row["parents"] = line.split("=",1)[1]
    return row
...
def print_status(jobs: list[JobSpec], tail_lines: int) -> None:
    ...
    for row in rows:
        print("="*100)
        print(f"{row['host']} {row['job_id']} running={row.get('running')} pid={row.get('pid','')} rows={row.get('rows','')} parents={row.get('parents','')} dim={row.get('dim','')}")
        print(str(row.get('raw','')).split('STATUS_LOG_TAIL_BEGIN',1)[-1].split('STATUS_LOG_TAIL_END',1)[0])
    print("="*100)
    w = csv.DictWriter(sys.stdout, fieldnames=["job_id","host","running","pid","rows","parents","dim","output_dir"])
    w.writeheader()
...
def stop_jobs(jobs: list[JobSpec]) -> None:
    for job in jobs:
        out_dir = shlex.quote(job.feature_dir)
        out = _ssh(job.host, f"OUT={out_dir}; PID=$(cat \"$OUT/child_feature_build.pid\" 2>/dev/null || true); if [ -n \"$PID\" ]; then kill -TERM \"$PID\" 2>/dev/null || true; fi; pkill -TERM -f \"$OUT\" 2>/dev/null || true; echo stopped out=$OUT pid=$PID", check=False)
        print(out.stdout, end="" if out.stdout.endswith("\n") else "\n")
...
def add_args(p: argparse.ArgumentParser) -> None:
    ...
    p.add_argument("--child-cache-dir", default="{root_dataset_dir}_child_transitions_adv")
    p.add_argument("--feature-dir", default="{child_cache_dir}_features_adv")
    p.add_argument("--num-processes", type=int, default=48)
    p.add_argument("--max-in-flight", type=int, default=0)
    p.add_argument("--flush-every-shards", type=int, default=256)
