...
void sync_python_meta_fields(SimState& state) {
    if (state.stats.next_adv_tick >= -1.0) {
        state.stats.decode_next_deadline_by_id[kPyMetaNextAdvTick] = state.stats.next_adv_tick;
    }
    if (state.stats.last_adv_tick >= -1.0) {
        state.stats.decode_next_deadline_by_id[kPyMetaLastAdvTick] = state.stats.last_adv_tick;
    }
    state.stats.decode_next_deadline_by_id[kPyMetaMissedAdvSource] = static_cast<double>(state.stats.missed_adv_source);
    state.stats.decode_tokens_counted_by_id[kPyMetaDecodeCreditBalance] = state.stats.decode_credit_balance;
}
...
std::unordered_map<int, ReqView> build_req_views(const SimState& state) {
    ...
    for (const auto& r : state.requests) {
        ...
        if (it_pref != state.stats.per_request_prefill_lateness_by_id.end()) {
            v.prefill_lateness = std::max(0.0, it_pref->second);
        } else if (v.prefill_done) {
            v.prefill_lateness = std::max(0.0, double(r.prefill_lateness));
        } else {
            v.prefill_lateness = std::max(0.0, double(state.sim_time) - v.prefill_deadline);
        }

        // Match Python _build_req_views(): only finalized stats count for decode
        // lateness; request-local decode_lateness is not a fallback signal here.
        const double dec_lateness = (it_dec != state.stats.per_request_decode_lateness_by_id.end())
            ? std::max(0.0, it_dec->second)
            : 0.0;
