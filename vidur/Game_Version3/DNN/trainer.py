...
def train_model_search(
    *,
    ...
    backend = str(extra_cfg.get("classical_backend", "")).strip()
    if backend == "symbolic_bellman":
        return train_symbolic_bellman_model(
            train_records=train_records,
            eval_records=eval_records,
            output_dir=output_dir,
        )
    if backend == "cliff_aware":
        from ..ModelSearchBed.cliff_aware_value_model import train_cliff_aware_model

        return train_cliff_aware_model(
            train_records=train_records,
            eval_records=eval_records,
            cfg=cfg,
            output_dir=output_dir,
    ...
    if backend == "neural":
        from ..ModelSearchBed.neural_value_model import train_neural_value_model

        return train_neural_value_model(
            train_records=train_records,
            eval_records=eval_records,
            cfg=cfg,
            output_dir=output_dir,
