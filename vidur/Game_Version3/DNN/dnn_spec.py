...
class DNNGameSpec:
    ...
    def validate(self) -> None:
        ...
        if self.d_req_emb <= 0 or self.d_global_emb <= 0 or self.d_trunk <= 0 or self.d_cond_emb <= 0:
            raise ValueError("embedding/trunk dimensions must be > 0")
        if self.num_attention_heads <= 0 or self.num_attention_layers <= 0:
            raise ValueError("attention settings must be > 0")
        if self.d_req_emb % self.num_attention_heads != 0:
            raise ValueError("d_req_emb must be divisible by num_attention_heads")
