...
This document describes the current Game Version 3 implementation in the Python GV3 pipeline. The main implementation lives under `vidur/Game_Version3/`.
...
GV3 uses fitted Bellman bootstrapping across generations. The bootstrap value should be generation-aware, not only model-version-aware.

For generation `0`, search must not use the randomly initialized model as a continuation value. All model bootstrap values are forced to zero:

```text
bootstrap_value = 0.0
```

This means controller roots in generation `0` learn the immediate one-step controller effect:

```text
Q_controller_gen0(s, a) = immediate_reward_or_cost(s, a)
```

Adversary roots are evaluated with the current GV3 two-ply structure: adversary action first, then the best controller response. In generation `0`, the adversary still gets this two-ply minimax target, but the terminal model bootstrap inside the controller response is zero:

```text
Q_adversary_gen0(s, a_adv) =
    max_a_ctrl [ immediate_controller_reward_or_cost(s_after_adv, a_ctrl) ]
```

In this game, the adversary action normally does not execute service and does not create immediate SLO cost. It only changes the request/decode-stop state at the adversary tick. Therefore the adversary action value is the selected controller-response value, not an extra adversary immediate reward plus that value.

From generation `1` onward, the model bootstrap is enabled at the leaf of the evaluated branch:

```text
Q = immediate_reward_or_cost + discount * V_model(next_state)
```

For controller roots, this expands the learned horizon by one Bellman step each generation. For adversary roots, the model bootstrap is used at the controller-response leaf. This means generation `1` adversary samples behave like a three-ply minimax target: adversary action, controller best response, then learned continuation value.

The implementation point is `mctsDNN.py`:

| Function | Bootstrap role |
| --- | --- |
| `_bootstrap_value_from_model` | Should return `0.0` when generation bootstrap is disabled |
| `_compose_q_from_state` | Combines immediate reward, discount, and bootstrap value |
| `_evaluate_adversary_action_q_two_step` | Uses controller best response; bootstrap occurs at the controller-response leaf |
| `search_dnn` | Should decide whether bootstrap is enabled from generation, not only model version |
...
vidur.Game_Version3.Network.server.run_experiment \
  --start-generation 0 \
  --num-generations 100 \
...
vidur.Game_Version3.tests.consolidated_frontier_tests \
  --num-roots 32
```
