...
def build_storage_config(
    *,
    ...
    if int(min_canonical_actions) < 0:
    ...
    return RootStorageConfig(
        ...
        min_canonical_actions=int(min_canonical_actions),
...
def root_has_min_canonical_actions(cfg: RootStorageConfig, mcts: Any, root: PreparedHistoryRoot) -> bool:
    """Return whether the root exposes enough canonical actions to be useful."""

    required = int(cfg.min_canonical_actions)
    if required <= 0:
        return True

    search_state = root.state.fork(flag=False)
    player = str(root.root_player)
    if player == "adversary":
        search_state = _build_root_decision_state_for_adversary(
            env=mcts._env,
            current_state=search_state,
    ...
    mask_values = mask_t.detach().cpu().tolist() if hasattr(mask_t, "detach") else list(mask_t)
    valid_indices = [
        int(idx)
        for idx, ok in enumerate(mask_values)
        if bool(ok) and idx < len(actions_by_index) and actions_by_index[idx] is not None
    ]
    ...
    return len(canonical_indices) >= required
        ...
        for batch in generate_candidate_root_batches(cfg, env):
            for root in batch:
                ...
                if not root_player_matches_filter(cfg, root.root_player):
                    continue
                if not root_has_min_canonical_actions(cfg, mcts, root):
                    continue
...
def parse_args() -> RootStorageConfig:
    ...
    return build_storage_config(
        ...
        min_canonical_actions=args.min_canonical_actions,
