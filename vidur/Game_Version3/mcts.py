...
from ..mcts.environment import VidurMCTSState
from ..mcts.game_types import AdversaryAction, ControllerAction
from ..mcts.launch_mcts_job import MCTSExploreConfig
...
class VanillaMCTS:
    ...
    def get_transition_reward(self, parent_node: Node, child_node: Node) -> float:
        # reward is the negative of the cost difference between the parent and child node states, essentially the negative of the incremental cost from parent to child
        return parent_node.state_cost - child_node.state_cost
    ...
    ### NEED TO DOUBLE CHECK THIS CODE FROM HERE ALL !

    def _expand_one_child(self, node: Node, state: VidurMCTSState) -> Tuple[Node, VidurMCTSState]:

        """
            Randomly selects one of the untried actions from the node and applies it from its state and gets a child
        ...
        action_idx = int(node.untried_action_indices.pop(pos))
        ...
        child = Node(
            player=self.get_next_player(node.player),
    ...
    def search_dnn(
        ...
        *,
        ...
        iterations = int(mcts_iter or getattr(self._mctsConfig, "num_simulations", 1000) or 1000)
        ...
        if not root.untried_action_indices and not root.children:
            return MCTSSearchResult(
                ...
                next_player=self.get_next_player(root.player),
