...
class SelfPlayRunner:
        ...
        try:
            ...
            if native_on and native_log_events:
                ...
                if it_path is not None:
                    ...
                    p_native = p.with_name(f"{p.stem}_native{suffix}")
    ...
    # TODO : Might be useful to move these functions into UTILs
    ...
    def _build_root_decision_state_for_adversary(
        ...
        *,
        ...
        """
        Root-level replay basis for adversary when controller crossed an adversary tick.
        Returns (search_state, forbidden_stop_ids).
        """
        ...
        # Only controller-crossed missed tick should use replay basis.
        if miss_src != 1:
            return current_state, set()
        ...
        # Align replay basis time to current adversary tick boundary.
        try:
        ...
        # Hide blocked ids from adversary sampler/features in replay decision state.
    ...
    def _sanitize_adversary_action_for_state(
        self,
        ...
        *,
        ...
        req_lookup = self.env._build_request_lookup(state.simulator, state=state)

        seen: Set[int] = set()
        stop_ids_clean: list[int] = []

        for rid_raw in (action.stop_decode_ids or []):
            rid = int(rid_raw)
            if rid in seen:
                continue
            seen.add(rid)
            ...
            try:
                rem_dec = int(self.env._remaining_decode(req))
            except Exception:
                rem_dec = max(
                    0,
                    int(getattr(req, "_num_decode_tokens", getattr(req, "num_decode_tokens", 0)))
                    - int(getattr(req, "num_processed_decode_tokens", 0)),
                )

            if rem_dec <= 0:
                continue

            stop_ids_clean.append(rid)

        if stop_ids_clean == list(action.stop_decode_ids or []):
            return action

        return AdversaryAction(
            requests=list(action.requests or []),
            stop_decode_ids=stop_ids_clean,
        )
    ...
    def run_n_roots(
        ...
        *,
        ...
        for k in range(int(num_roots)):
            ...
            ## Creating Decision State for Adv is applicable 
            search_root_state = state
            replay_forbidden_stop_ids: Set[int] = set()
            if player == "adversary":
                search_root_state, replay_forbidden_stop_ids = self._build_root_decision_state_for_adversary(
                    current_state=state,
            ...
            if action is None:
                actions_by_index, mask = self._sample_actions_readonly(search_root_state, player)
            ...
            try:
                if player_acted == "adversary":
                    ...
                    decision_state_time_for_row = float(state.simulator._time)
            except Exception:
                decision_state_time_for_row = float(state.simulator._time)

            pre_controller_snapshot = None
            pre_controller_stats = None
            if player_acted == "controller":
            ...
            if player_acted == "adversary":
                action = self._sanitize_adversary_action_for_state(
                    state,
                    action,
            ...
            # This row represents the chosen root action after it has been applied.
            root_logger = getattr(self.mcts, "_root_logger", None)
            if root_logger is not None and root is not None and hasattr(root_logger, "log_root"):
                ...
                root_logger.log_root(
                    game_id=int(game_id),
                    root_id=int(root_id),
                    root_depth=int(root_depth_before),
                    ...
                    decision_state_time=float(decision_state_time_for_row),
    ...
    def _valid_action_indices_readonly(self, state: VidurMCTSState, player: str) -> list[int]:
        try:
        ...
        for i, ok in enumerate(mask_list):
            if not ok:
                continue
            if i >= len(actions_by_index):
                continue
            if actions_by_index[i] is None:
                continue
            valid.append(int(i))
        return valid

    def _align_player_to_valid_actions(self, state: VidurMCTSState, player: str) -> str:
        p = str(player)
        if self._valid_action_indices_readonly(state, p):
            return p
        other = "controller" if p == "adversary" else "adversary"
        if self._valid_action_indices_readonly(state, other):
            return other
        return p
    ...
    def _pick_mcts_action_for_arena_step(
        ...
        *,
        ...
        search_state: Optional[VidurMCTSState] = None,
        forbidden_stop_ids: Optional[Set[int]] = None,
    ):
        search_basis = search_state if search_state is not None else state
        actions_by_index, mask = self._sample_actions_readonly(search_basis, player)
        valid_indices = [
            int(i)
            for i, ok in enumerate(_mask_to_list(mask))
            if ok and i < len(actions_by_index) and actions_by_index[i] is not None
        ]
        iterations_requested = int(iterations)

        def _meta(mode: str, it_used: int) -> dict:
            return {
                "selection_mode": str(mode),  # "mcts" | "single_action_shortcut"
                "valid_action_count": int(len(valid_indices)),
                "iterations_requested": int(iterations_requested),
                "iterations_used": int(it_used),
            }

        def _fallback_idx() -> int:
            fallback = list(valid_indices)
            if player == "adversary" and bool(prefer_nonempty_adversary):
                nonempty = []
                for i in fallback:
                    a = actions_by_index[i]
                    if isinstance(a, AdversaryAction) and len((a.requests or [])) > 0:
                        nonempty.append(i)
                if nonempty:
                    fallback = nonempty
            ...
            if player == "controller":
                ...
                for i in fallback:
                    a = actions_by_index[i]
                    if isinstance(a, ControllerAction) and bool(getattr(a, "prefill_allocations", {}) or {}):
                        with_prefill.append(i)
                if with_prefill:
                    fallback = with_prefill
                else:
                    non_noop = []
                    for i in fallback:
                        a = actions_by_index[i]
                        if isinstance(a, ControllerAction):
                            budget = int(getattr(a, "token_budget", 0) or 0)
                            if budget > 0:
                                non_noop.append(i)
                    if non_noop:
                        fallback = non_noop
        ...
        # Short-circuit: no search needed if <=1 valid action.
        if len(valid_indices) == 0:
            return None, -1, _meta("single_action_shortcut", 0)
        ...
        if len(valid_indices) == 1:
            ...
            return action, idx, _meta("single_action_shortcut", 0)

        # Branching state: run MCTS.
        search_state_fork = search_basis.fork(flag=False)

        self.mcts.search_dnn(
            dnn_model=model,
            rootState=search_state_fork,
            root_player=player,
        ...
        try:
            ...
            if root is None or not root.children:
                idx = _fallback_idx()
                if idx < 0:
                    return None, -1, _meta("mcts", iterations_requested)
                act = actions_by_index[idx]
                if player == "adversary" and act is not None:
                    act = self._sanitize_adversary_action_for_state(
                        state,
                        act,
                        forbidden_stop_ids=forbidden_stop_ids,
                    )
                return act, idx, _meta("mcts", iterations_requested)

            candidate_idxs = [
                int(i)
                for i, ch in root.children.items()
                if ch is not None and i in valid_indices
            ]
            ...
            action = actions_by_index[int(best_idx)] if int(best_idx) < len(actions_by_index) else None
            if action is None:
                return None, -1, _meta("mcts", iterations_requested)
            if player == "adversary":
                action = self._sanitize_adversary_action_for_state(
            ...
            return action, int(best_idx), _meta("mcts", iterations_requested)
    ...
    def _run_arena_cycle(
        ...
        *,
        ...
        while (
            ...
            player = self._align_player_to_valid_actions(state, player)
            ...
            replay_forbidden_stop_ids: Set[int] = set()
            if player == "adversary":
                search_basis_state, replay_forbidden_stop_ids = self._build_root_decision_state_for_adversary(
                    current_state=state,
                    pre_controller_snapshot=pending_adv_pre_ctrl_snapshot,
                    pre_controller_stats=pending_adv_pre_ctrl_stats,
                )
            ...
            action, _, selection_info = self._pick_mcts_action_for_arena_step(
                state=state,
                player=player,
                model=model,
                game_id=int(game_id),
            ...
            if cycle_file_logger is not None:
                cycle_file_logger.write_step(
                    ...
                    **state_log,
                    ...
                    iterations_requested=int(selection_info.get("iterations_requested", iters)),
            ...
            if cycle_file_logger is not None:
                cycle_file_logger.write_step(
                    ...
                    **state_log,
                    selection_mode=str(selection_info.get("selection_mode", "")),
                    valid_action_count=int(selection_info.get("valid_action_count", 0)),
                    iterations_requested=int(selection_info.get("iterations_requested", cont_iterations_per_root)),
                    iterations_used=int(selection_info.get("iterations_used", 0)),
                )
    ...
    def run_arena_game(
        ...
        *,
        ...
        # Arena start-state guard:
        # 1) if current side has no legal actions but the other side does, realign player.
        # 2) if neither side has legal actions after history, fall back to fresh initial state.
        player = self._align_player_to_valid_actions(state, player)
        if not self._valid_action_indices_readonly(state, player):
            state = self.env.initial_state()
            player = self._align_player_to_valid_actions(state, str(start_player))
            depth = int(start_root_depth)
