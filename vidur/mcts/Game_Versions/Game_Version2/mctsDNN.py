...
try:
    from ... import mcts_native_gv2 as _mcts_native
except Exception:
    _mcts_native = None
...
class VidurMCTS:
    ...
    def __init__(
        ...
        *,
        ...
        setattr(self._env, "_enable_internal_step_logging", bool(self._iter_log_enabled))
    ...
    def _expand_node(self, node: MCTSNode, state: VidurMCTSState, dnn_model: Any) -> Tuple[float, bool, int]:
        ...
        decision_state, next_tick = self._decision_state(node, state, node.player)
        node.last_decision_state_time = float(decision_state.simulator._time)
        forbidden_stop_ids = self._adversary_replay_forbidden_stop_ids(node, decision_state, state)
        actions_by_index, mask = self._actions_and_mask(decision_state, node.player, forbidden_stop_ids=forbidden_stop_ids)
        ...
        if mode == "uniform":
            ...
            model_value, priors = self._nn_value_and_priors(
                dnn_model, decision_state, node.player, action_mask=mask.unsqueeze(0)
            )
            used_model = True
    ...
    def _normalize_local_q(q_controller: float, q_min: Optional[float], q_max: Optional[float]) -> float:
        if q_min is None or q_max is None:
            return 0.0
        den = float(q_max) - float(q_min)
        if den <= 1e-8:
            return 0.0
        qn = (float(q_controller) - float(q_min)) / den
        return max(0.0, min(1.0, qn))
    ...
    def _advance_through_single_child_chain(
        ...
        *,
        forced_step_logs: Optional[List[Tuple[MCTSNode, Dict[str, Any], int, int, str]]] = None,
        max_hops: int = 2000,
    ) -> Tuple[MCTSNode, VidurMCTSState]:
        """
        Advance through deterministic (single-valid-action) nodes until we reach
        branching (>1 valid) or terminal (0 valid).

        Important GV2 behavior:
        - Uses _decision_state(...) for ACTION SAMPLING (missed adversary tick handling).
        - Applies chosen action on the real path `state` to preserve simulator progression.
        """
        hops = 0
        while hops < max_hops:
            decision_state, _ = self._decision_state(node, state, node.player)
            forbidden_stop_ids = self._adversary_replay_forbidden_stop_ids(node, decision_state, state)
            actions_by_index, mask = self._actions_and_mask(decision_state, node.player, forbidden_stop_ids=forbidden_stop_ids)
            valid = [i for i, ok in enumerate(mask) if ok and actions_by_index[i] is not None]
            n_valid = len(valid)
            ...
            if forced_step_logs is not None:
                forced_action = node.parent_action
                forced_adv_deadlines = "{}"
                if isinstance(forced_action, AdversaryAction):
                    forced_adv_deadlines = self._adversary_prefill_deadlines_by_id_json(state, forced_action)

                forced_step_logs.append(
                    (
                        node,
                        self._env.describe_state(state),
                        int(n_valid),
                        int(len(node.children)),
                        forced_adv_deadlines,
                    )
                )
            ...
            if child is None:
                ...
                if child.parent_action_index is None:
    ...
    def run_one_simulation(
        ...
        # Clearning interval events for decode debugging :
        clear_events_fn = getattr(self._env, "clear_internal_events", None)
        if callable(clear_events_fn):
            clear_events_fn()
        ...
        if iter_logging:
            ...
            # 1. Forced Steps ##
            for forced_node, forced_snap, forced_n_valid, forced_unique, forced_adv_deadlines in forced_logs:
                ...
                payload = self._action_log_payload(forced_node.parent_action)
                self._iter_logger.log_expand(
                    ...
                    decision_state_time=float(getattr(forced_node, "last_decision_state_time", forced_snap.get("sim_time", 0.0))),
                    ...
                    adversary_prefill_deadlines_by_id_json=forced_adv_deadlines,
                    **payload,
                )
            ...
            self._iter_logger.log_expand(
                ...
                decision_state_time=float(getattr(leaf_node, "last_decision_state_time", leaf_state.simulator._time)),
        ...
        if iter_logging:
            self._flush_internal_env_events(
                root=root,
                leaf_node=leaf_node,
                game_id=game_id,
                root_id=root_id,
    ...
    def _native_mcts_enabled(self) -> bool:
        ...
        fn = getattr(_mcts_native, "search_mcts_dnn_torchscript", None)
        if fn is None:
            raise RuntimeError("mcts_native_gv2.search_mcts_dnn_torchscript missing")
        doc = str(getattr(fn, "__doc__", ""))
        if ("root_state_payload" not in doc) or ("cfg_payload" not in doc):
            raise RuntimeError("Loaded native module exposes old API; expected GV2 payload API")
    ...
    def _search_dnn_native(
        ...
        *,
        ...
        if _mcts_native is None:
            raise RuntimeError("native_mcts_enabled=True but native module is unavailable")
        if not hasattr(_mcts_native, "search_mcts_dnn_torchscript"):
            raise RuntimeError(
                "mcts_native_gv2.search_mcts_dnn_torchscript is missing; rebuild native module."
            )
        ...
        alpha_cfg = getattr(self._cfg, "reward_tail_alpha", None)
        reward_tail_alpha = float(alpha_cfg) if alpha_cfg is not None else (1.0 / headroom)
        ...
        def _tensor_1d_f32(t: Any) -> list[float]:
            if t is None:
                return []
            tt = t.detach().to(dtype=torch.float32).cpu()
            if tt.dim() == 2 and tt.shape[0] == 1:
                tt = tt.squeeze(0)
            return [float(x) for x in tt.reshape(-1).tolist()]

        def _tensor_1d_bool(t: Any) -> list[bool]:
            if t is None:
                return []
            tt = t.detach().to(dtype=torch.bool).cpu()
            if tt.dim() == 2 and tt.shape[0] == 1:
                tt = tt.squeeze(0)
            return [bool(x) for x in tt.reshape(-1).tolist()]

        def _tensor_2d_f32_with_shape(t: Any) -> tuple[list[list[float]], int, int]:
            if t is None:
                return [], 0, 0
            tt = t.detach().to(dtype=torch.float32).cpu()
            if tt.dim() == 3 and tt.shape[0] == 1:
                tt = tt.squeeze(0)
            elif tt.dim() == 1:
                tt = tt.unsqueeze(0)
            elif tt.dim() > 2:
            ...
            n = int(tt.shape[0])
            d = int(tt.shape[1])
            return tt.tolist(), n, d

        # Build root action mask exactly from env sampler
        if str(root_player) == "controller":
            _, mask = self._env.sample_controller_actions(rootState)
        else:
            _, mask = self._env.sample_adversary_actions(rootState)
        if not isinstance(mask, torch.Tensor):
            mask = torch.tensor(mask, dtype=torch.bool)
        else:
            mask = mask.to(dtype=torch.bool)

        # Build NN inputs from current root state (same feature builder as Python path)
        inputs = dnn_infer.build_model_inputs(
            rootState,
            str(root_player),
            torch.device("cpu"),
        ...
        inputs = replace(inputs, action_mask=mask.unsqueeze(0))

        prefill_feat, prefill_n, prefill_d = _tensor_2d_f32_with_shape(inputs.prefill_req_features)
        decode_feat, decode_n, decode_d = _tensor_2d_f32_with_shape(inputs.decode_req_features)
        legacy_req_feat, req_n, req_d = _tensor_2d_f32_with_shape(inputs.req_features)

        prefill_mask = _tensor_1d_bool(inputs.prefill_req_mask)
        decode_mask = _tensor_1d_bool(inputs.decode_req_mask)
        legacy_req_mask = _tensor_1d_bool(inputs.req_mask)
        action_mask = _tensor_1d_bool(inputs.action_mask)
        global_features = _tensor_1d_f32(inputs.global_features)

        sim_time_now = float(rootState.simulator._time)
        if str(root_player).strip().lower() == "adversary":
            try:
                decision_state_time = float(self._env._v2_current_adv_tick(rootState))
            except Exception:
        ...
        active_ids = sorted(int(x) for x in (getattr(stats, "active_request_ids", set()) or set()))
        completed_ids = sorted(int(x) for x in (getattr(stats, "completed_request_ids", set()) or set()))
        dropped_ids = set(int(x) for x in (getattr(stats, "dropped_request_ids", set()) or set()))
        stopped_ids = set(int(x) for x in (getattr(stats, "stopped_decode_request_ids", set()) or set()))
        violated_ids = set(int(x) for x in (getattr(stats, "violated_request_ids", set()) or set()))
        prefill_finalized_ids = sorted(int(x) for x in (getattr(stats, "prefill_lateness_finalized", set()) or set()))

        per_prefill_lateness = {
            int(k): float(v)
            for k, v in dict(getattr(stats, "per_request_prefill_lateness", {}) or {}).items()
        }
        per_decode_lateness = {
            int(k): float(v)
            for k, v in dict(getattr(stats, "per_request_decode_lateness", {}) or {}).items()
        }
        decode_next_deadline = {
            int(k): float(v)
            for k, v in dict(getattr(stats, "decode_next_deadline_by_id", {}) or {}).items()
        }
        decode_counted = {
            int(k): int(v)
            for k, v in dict(getattr(stats, "decode_tokens_counted", {}) or {}).items()
        }

        req_lookup = self._env._build_request_lookup(rootState.simulator, state=rootState)
        requests_payload: list[dict[str, Any]] = []
        for rid in sorted(int(x) for x in req_lookup.keys()):
            req = req_lookup.get(rid)
            if req is None:
                continue

            arrived_at = _to_float(getattr(req, "arrived_at", 0.0), 0.0)
            queued_at = _to_float(getattr(req, "queued_at", arrived_at), arrived_at)

            num_prefill_tokens = _to_int(
                getattr(req, "_num_prefill_tokens", getattr(req, "num_prefill_tokens", 0)),
                0,
            )
            num_decode_tokens = _to_int(
                getattr(req, "_num_decode_tokens", getattr(req, "num_decode_tokens", 0)),
                0,
            )
            num_processed_prefill_tokens = _to_int(getattr(req, "num_processed_prefill_tokens", 0), 0)
            num_processed_decode_tokens = _to_int(getattr(req, "num_processed_decode_tokens", 0), 0)

            prefill_slo_time = _to_float(
                getattr(req, "_prefill_slo_time", getattr(req, "prefill_slo", 0.0)),
                0.0,
            )
            decode_slo_time = _to_float(
                getattr(req, "_decode_slo_time", getattr(req, "decode_slo", 0.0)),
                0.0,
            )

            prefill_deadline = arrived_at + prefill_slo_time if prefill_slo_time > 0.0 else 0.0
            decode_deadline = float(
                decode_next_deadline.get(
                    rid,
                    (queued_at + decode_slo_time) if decode_slo_time > 0.0 else 0.0,
                )
            )
            ...
            dropped = (rid in dropped_ids)
            stopped_decode = (rid in stopped_ids)
            violated = (rid in violated_ids)

            completed = bool(getattr(req, "completed", False))
            is_prefill_complete = bool(getattr(req, "_is_prefill_complete", getattr(req, "is_prefill_complete", False)))

            prefill_completed_at = float(getattr(req, "_prefill_completed_at", 0.0)) if is_prefill_complete else 0.0
            completed_at = float(getattr(req, "_completed_at", 0.0)) if completed else 0.0
            ...
            requests_payload.append(
                {
                    ...
                    "num_processed_prefill_tokens": int(num_processed_prefill_tokens),
                    "num_decode_tokens": int(num_decode_tokens),
                    "num_processed_decode_tokens": int(num_processed_decode_tokens),
                    "prefill_slo_time": float(prefill_slo_time),
                    "decode_slo_time": float(decode_slo_time),
                    "completion_slo_time": 0.0,
                    "prefill_deadline": float(prefill_deadline),
                    "decode_next_deadline": float(decode_deadline),
                    "prefill_completed_at": _to_float(prefill_completed_at, 0.0),
                    "completed_at": _to_float(completed_at, 0.0),
                    "prefill_lateness": float(per_prefill_lateness.get(rid, 0.0)),
                    "decode_lateness": float(per_decode_lateness.get(rid, 0.0)),
                    "is_prefill_complete": bool(is_prefill_complete),
                    "completed": bool(completed),
                    "dropped": bool(dropped),
                    "stopped_decode": bool(stopped_decode),
                    "violated": bool(violated),
                }
            )

        next_request_id = _to_int(getattr(rootState.simulator, "_next_request_id", -1), -1)
        if next_request_id < 0:
            sched = getattr(rootState.simulator, "_scheduler", None)
            next_request_id = _to_int(getattr(sched, "_next_request_id", -1), -1)
        if next_request_id < 0:
            universe = list(active_ids) + list(completed_ids) + [int(x) for x in req_lookup.keys()]
            next_request_id = (max(universe) + 1) if universe else 0

        pending_adv_tick = False
        last_adv_tick = -1.0
        next_adv_tick = sim_time_now
        missed_adv_source = 0
        try:
            pending_adv_tick = bool(self._env._v2_has_pending_adv_tick(rootState))
            next_adv_tick = float(self._env._v2_next_adv_tick(rootState))
            missed_adv_source = int(self._env._v2_missed_adv_source(rootState))
            last_adv_tick = float(
                self._env._v2_meta_get(rootState.stats, self._env._META_LAST_ADV_TICK, -1.0)
            )
        except Exception:
            pass
        ...
        try:
            decode_credit_balance = int(self._env._v2_decode_credit_balance_raw(rootState))
            decode_credit_available = int(self._env._v2_decode_credit_balance(rootState))
        except Exception:
            decode_credit_balance = _to_int(getattr(stats, "decode_credit_balance", 0), 0)
            decode_credit_available = max(0, decode_credit_balance)

        recent_arrivals_raw = list(getattr(stats, "recent_arrivals", []) or [])
        recent_arrivals_ts: list[float] = []
        recent_launches: list[tuple[float, int, int]] = []
        for item in recent_arrivals_raw:
            if isinstance(item, (tuple, list)):
                if len(item) >= 3:
                    ts = _to_float(item[0], 0.0)
                    cnt = max(0, _to_int(item[1], 0))
                    pref = max(0, _to_int(item[2], 0))
                    recent_arrivals_ts.append(ts)
                    recent_launches.append((ts, cnt, pref))
                elif len(item) >= 1:
                    recent_arrivals_ts.append(_to_float(item[0], 0.0))
            elif isinstance(item, (int, float)):
                recent_arrivals_ts.append(float(item))

        root_state_payload = {
            "contract_version": "gv2_native_phase1_v1",
            ...
            "stats": {
                "requests_generated": int(getattr(stats, "requests_generated", 0)),
                "requests_completed": int(getattr(stats, "requests_completed", 0)),
                "slo_violations": int(getattr(stats, "slo_violations", 0)),
                "slo_lateness_sum": float(getattr(stats, "slo_lateness_sum", 0.0)),
                "recent_arrivals": recent_arrivals_ts,
                "recent_launches": recent_launches,
                "active_request_ids": active_ids,
                "completed_request_ids": completed_ids,
                "dropped_request_ids": sorted(int(x) for x in dropped_ids),
                "stopped_decode_request_ids": sorted(int(x) for x in stopped_ids),
                "violated_request_ids": sorted(int(x) for x in violated_ids),
                "prefill_lateness_finalized_ids": prefill_finalized_ids,
                "per_request_prefill_lateness_by_id": per_prefill_lateness,
                "per_request_decode_lateness_by_id": per_decode_lateness,
                "decode_next_deadline_by_id": decode_next_deadline,
                "decode_tokens_counted_by_id": decode_counted,
                "decode_credit_balance": int(decode_credit_balance),
                "decode_credit_available": int(decode_credit_available),
            ...
            "prefill_req_n": int(prefill_n),
            "prefill_req_d": int(prefill_d),
            "decode_req_n": int(decode_n),
            "decode_req_d": int(decode_d),
            "req_features": legacy_req_feat,
            "req_mask": legacy_req_mask,
            "req_n": int(req_n),
            "req_d": int(req_d),
        }

        gv2_cfg = getattr(self._env, "_gv2_cfg", None)
        cfg_payload: dict[str, Any] = {
            ...
            "discount_time_denominator_sec": float(getattr(self, "_discount_time_denom", getattr(self, "_prefill_step_time", 0.015725797204323228))),
            "reward_knee": float(reward_knee),
            "reward_max_penalty": float(reward_max_penalty),
            "reward_tail_alpha": float(reward_tail_alpha),
            "reuse_root_infer_inputs": bool(getattr(self._cfg, "reuse_root_infer_inputs", False)),
            "native_log_flush_every": int(getattr(self._cfg, "native_log_flush_every", 1)),
            "fallback_total_time_sec": float(getattr(self, "_prefill_step_time", 0.015725797204323228)),
            "fallback_model_time_sec": float(getattr(self, "_prefill_step_time", 0.015725797204323228)),
        }

        if gv2_cfg is not None:
            decode_slos = tuple(getattr(getattr(gv2_cfg, "legacy_mcts", None), "decode_slos", ()) or (50.0,))
            ...
            cfg_payload.update(
                {
                    ...
                    "prefill_window_cap_tokens": int(getattr(self._env, "_prefill_window_cap_tokens", 0)),
                    "max_prefill_tokens_per_request": int(gv2_cfg.request.max_prefill_tokens_per_request),
                    "max_decode_tokens_per_request": int(gv2_cfg.request.max_decode_tokens_per_request),
                    "min_decode_tokens_per_request": int(gv2_cfg.request.min_decode_tokens_per_request),
                    "decode_slo_time_default": float(decode_slo_default),
                    "auto_drop_lateness_sec": float(gv2_cfg.cost.auto_drop_lateness_sec),
                    "drop_cost": float(gv2_cfg.cost.drop_cost),
                    "controller_noop_prefill_only_jump_to_next_adv_tick": bool(
                        gv2_cfg.timing.controller_noop_prefill_only_jump_to_next_adv_tick
                    ),
                    "enforce_nonnegative_decode_credits": bool(gv2_cfg.credits.enforce_nonnegative_decode_credits),
                    "decode_credit_mint_per_prefill_complete": int(gv2_cfg.credits.decode_credit_mint_per_prefill_complete),
                }
            )

        constraints = getattr(self._env, "_constraints", None)
        predictor_csv = str(getattr(constraints, "prefill_profile_path", "") or "")
        if predictor_csv:
            cfg_payload["native_predictor_csv_path"] = predictor_csv

        prof = getattr(self._env, "_prefill_profile", None)
        entries = dict(getattr(prof, "entries", {}) or {})
        if entries:
            tok = sorted(int(k) for k in entries.keys())
            cfg_payload["prefill_profile_tokens"] = tok
            cfg_payload["prefill_profile_times"] = [float(entries[k]) for k in tok]

        def _native_sidecar_csv_path(path_like: str) -> str:
            p = Path(path_like)
            suffix = p.suffix if p.suffix else ".csv"
            return str(p.with_name(f"{p.stem}_native{suffix}"))

        iter_log_path = ""
        if bool(getattr(self, "_iter_log_enabled", False)):
            p = getattr(getattr(self, "_iter_logger", None), "_path", None)
            if p is not None:
                # Native iter logger uses a different schema than Python's canonical iter CSV.
                # Keep it in a sidecar file to avoid schema collisions.
                iter_log_path = _native_sidecar_csv_path(str(p))

        root_log_path = ""
        p_root = getattr(getattr(self, "_root_logger", None), "_path", None)
        if p_root is not None:
            # Native root logger also has a different schema; keep separate from
            # Python canonical root log rows written later in search_dnn().
            root_log_path = _native_sidecar_csv_path(str(p_root))

        seed = int(self._rng.randint(0, 2**31 - 1))
        profile_enabled = bool(getattr(self._cfg, "native_profile", False))
        log_events_enabled = bool(getattr(self._cfg, "native_log_events", True))

        native_result = _mcts_native.search_mcts_dnn_torchscript(
            ...
            root_id=int(root_id),
            seed=int(seed),
            log_events=bool(log_events_enabled),
        ...
        if isinstance(perf, dict) and profile_enabled:
            try:
                print(
                    "[NATIVE_SEARCH_PERF] "
    ...
    def search_dnn(self, dnn_model: Any, rootState: VidurMCTSState, root_player: str, iterations: int , * , game_id: int , root_id: int, root_node_id_override: int | None , root_depth: int , root_phase: str = "train_root", cycle_label: str = "",) -> Tuple[str, List[Union[AdversaryAction, ControllerAction]]]:
        ...
        if str(self._root.player).strip().lower() == "adversary":
            try:
                decision_state_time = float(self._env._v2_current_adv_tick(rootState))
            except Exception:
                decision_state_time = sim_time_now
    ...
    def _action_log_payload(self, action: Optional[Union[AdversaryAction, ControllerAction]]) -> Dict[str, Any]:
        payload = {
        ...
        if isinstance(action, AdversaryAction):
            ...
            payload["adversary_action_json"] = json.dumps(
                {
                    "type": "adversary",
                    "requests": reqs,
            ...
            payload["adversary_prefill_slos_json"] = json.dumps([float(r["prefill_slo"]) for r in reqs])
            payload["adversary_decode_slos_json"] = json.dumps([float(r["decode_slo"]) for r in reqs])
            return payload

        if isinstance(action, ControllerAction):
            tok = {int(k): int(v) for k, v in (action.token_allocations or {}).items()}
            pre = {int(k): int(v) for k, v in (action.prefill_allocations or {}).items()}
            dec = {int(k): int(v) for k, v in (action.decode_allocations or {}).items()}
            sel = [int(x) for x in (action.selected_request_ids or list(tok.keys()))]
            payload.update(
                {
                    "controller_token_budget": int(action.token_budget),
                    "controller_selected_ids_json": json.dumps(sel),
                    "controller_allocations_json": json.dumps(tok),
                    "controller_prefill_allocations_json": json.dumps(pre),
                    "controller_decode_allocations_json": json.dumps(dec),
                    "controller_prefill_total": int(sum(pre.values())),
                    "controller_decode_total": int(sum(dec.values())),
                    "controller_heuristic": str(action.heuristic or ""),
                    "controller_strategy": str(action.strategy or ""),
                }
            )
        return payload
    ...
    def _compute_root_log_payload_without_nn(
        ...
        *,
        ...
        if root.player == "controller":
            _, mask_raw = self._env.sample_controller_actions(root_state)
        else:
            _, mask_raw = self._env.sample_adversary_actions(root_state)
    ...
    def _flush_internal_env_events(
        self,
        *,
        root: MCTSNode,
        leaf_node: MCTSNode,
        ...
        sim_iteration: Optional[int],
    ) -> None:
        drain_fn = getattr(self._env, "drain_internal_events", None)
        ...
        if self._iter_logger is None or not hasattr(self._iter_logger, "log_internal_step"):
            return

        gid = int(game_id) if game_id is not None else 0
        rid = int(root_id) if root_id is not None else 0
        sid = int(sim_iteration) if sim_iteration is not None else -1

        for seq, ev in enumerate(events, start=1):
            if not isinstance(ev, dict):
                continue
            snap = ev.get("state_snapshot", {})
            if not isinstance(snap, dict):
                snap = {}

            self._iter_logger.log_internal_step(
                game_id=gid,
                root_id=rid,
                sim_iteration=sid,
                root_depth=int(root.depth),
                root_node_id=int(root.node_id),
                root_player=str(root.player),
                event_seq=int(seq),
                event_id=int(ev.get("event_id", seq)),
                phase=str(ev.get("phase", "internal:unknown")),
                node_id=int(leaf_node.node_id) if leaf_node is not None else None,
                parent_node_id=(leaf_node.parent.node_id if (leaf_node is not None and leaf_node.parent is not None) else None),
                start_time=float(ev.get("start_time", snap.get("sim_time", 0.0))),
                end_time=float(ev.get("end_time", snap.get("sim_time", 0.0))),
                stage_total_time=float(ev.get("stage_total_time", 0.0)),
                reason=str(ev.get("reason", "")),
                request_ids=[int(x) for x in ev.get("request_ids", [])],
                num_tokens=[int(x) for x in ev.get("num_tokens", [])],
                decode_credit_before=(None if ev.get("decode_credit_before", None) is None else int(ev.get("decode_credit_before"))),
                decode_credit_after=(None if ev.get("decode_credit_after", None) is None else int(ev.get("decode_credit_after"))),
                state_snapshot=snap,
            )
