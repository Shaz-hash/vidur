...
def _evaluate_samples(
    ...
    *,
    ...
    for i in range(0, len(samples), bsz):
        ...
        mixed = collate_mixed_samples(chunk, device=trainer.device)
...
def _selfplay_worker_main(
    ...
    try:
        ...
        runner.run_n_roots(
            ...
            progress_prefix=f"[GV3 selfplay gen={int(gen):06d} worker={int(worker_id):02d}]",
...
def run_parallel_self_improvement(cfg: MultipleProcessTrainingConfig) -> None:
    ...
    try:
        ...
        for local_gen in range(int(cfg.num_generations)):
            ...
            reuse_dataset_generation = int(getattr(cfg, "reuse_dataset_generation", -1))
            if reuse_dataset_generation >= 0:
                ...
                for wid, (start, count) in enumerate(splits):
                    ...
                    eval_split_seed = int(cfg.eval_split_seed_base) + gen * 100_000 + wid
                    ...
                    task_payloads.append(
                        {
                            ...
                            "task_kind": "selfplay",
                            "generation": gen,
                            "out_dir_train": str(gen_train_dir / f"proc_{wid:02d}"),
                            "out_dir_eval": str(gen_eval_dir / f"proc_{wid:02d}"),
                            ...
                            "action_seed_base": int(action_seed_base),
                ...
                for task in task_payloads:
                    worker_id = int(task.get("worker_id", 0))
                    p = ctx.Process(
                        target=_selfplay_worker_main,
                        args=(worker_id, cfg, task, result_q),
                        daemon=False,
                    )
                    p.start()
                    workers.append(p)

                worker_msgs = _collect_results(
                    result_q,
                    expected=tasks_sent,
                    task_kind="selfplay",
                ...
                generation_stats = _aggregate_worker_run_stats(worker_msgs)
                print(
            ...
            if not math.isnan(select_loss):
                trainer.maybe_save_best(select_loss, best_ckpt_path)

            common_row = {
                "generation": int(gen),
                "model_version": int(gen),
                "num_roots_required": int(cfg.roots_per_generation),
