...
# vidur/vidur/mcts/tests/DNN_TESTS/Game_Version2/mcts_tests.py
...
def test_missed_tick_replay_correctness_gv2(
    ...
    *,
    ...
    # Controller rows are logged on resulting post-transition state.
    ...
    # not decision-time accurate. Keep this check strict on no-op controller rows.
    ...
    sim_t = float(getattr(row, "decision_state_time", row.sim_time))
    ...
    if pending_logged != pending_expected:
        _fail(
            ...
            (
                ...
                f"(last_adv_tick={float(last_tick):.9f}, next_tick={next_tick:.9f}, decision_state_time={sim_t:.9f})"
            ),
            row,
            trace_id,
        )
