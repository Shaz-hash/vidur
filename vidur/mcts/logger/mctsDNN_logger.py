...
class DNNMCTSIterationLogger:
    ...
    PUCT_FIELDS = [
        ...
        "reward",
        "node_dnn_value",
        "children_created_json",      # [[child_node_id, prior, action_index], ...]
        "dedup_children_json",        # [[canonical_child_id, added_prior, alias_action_indices], ...]
        "ancestor_chain_json",        # [{"node_id":..,"parent_node_id":..,"value_sum":..,"visits":..,"mean_value":..}, ...]
        "selection_trace_json",       # per-hop PUCT candidates/scores/choice
        "minmax_min",
        "minmax_max",
    ]
    ...
    def _ensure_puct(self) -> None:
        if not self._path:
            return
        if getattr(self, "_puct_writer", None) is not None:
            return
