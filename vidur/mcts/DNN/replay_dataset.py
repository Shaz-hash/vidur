...
from .replay_write import ACTION_SPACE_SIZE, RootSample
...
class ReplayShardSampler:
    ...
    def sample_one(self, *, player: Optional[str] = None) -> RootSample:
        ...
        """
        if player is not None and player not in ACTION_SPACE_SIZE:
...
def collate_player_samples(
    ...
    *,
    ...
    """
    Collate samples for ONE player into a batch.
    Returns tensors ready for model.forward().

    Stored tensors are CPU and WITHOUT batch dimension.
    """
    if not samples:
    ...
    a = ACTION_SPACE_SIZE[player]

    req_features = torch.stack([s["inputs"]["req_features"] for s in samples], dim=0).to(device)
    global_features = torch.stack([s["inputs"]["global_features"] for s in samples], dim=0).to(device)

    req_mask_list = [s["inputs"].get("req_mask") for s in samples]
    if any(m is not None for m in req_mask_list):
        req_mask = _stack_or_default_bool(req_mask_list, shape=(len(samples), req_features.shape[1])).to(device)
    else:
        req_mask = None
    ...
    # Ensure policy is masked + normalized over valid actions (SAFETY CHECK)
