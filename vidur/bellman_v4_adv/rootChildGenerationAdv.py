...
from vidur.mcts.Game_Versions.Game_Version3.ModelSearchBed.self_model_test import (
    RootStateLoader,
...
def build_transition_trace_row(
    *,
    ...
    parent_snapshot: Any,
    parent_stats: Any,
    intermediate_snapshot: Any,
    intermediate_stats: Any,
    child_snapshot: Any,
    child_stats: Any,
    controller_action_index: int,
    controller_canonical_action_index: int,
    controller_alias_action_indices: list[int],
    controller_action_repr: str,
    adversary_action_index: int,
    adversary_action_repr: str,
    parent_cost: float,
    ...
    next_player: str,
    num_controller_valid_actions: int,
    num_controller_canonical_actions: int,
    num_adversary_valid_actions: int,
) -> dict[str, Any]:
    row: dict[str, Any] = {
        "schema_version": SCHEMA_VERSION,
        "transition_type": TRANSITION_TYPE,
        "trace_type": "root_child_transition_adv",
        ...
        # Backward-compatible action fields identify the controller action.
        "action_index": int(controller_action_index),
        "canonical_action_index": int(controller_canonical_action_index),
        "alias_action_indices": [int(x) for x in controller_alias_action_indices],
        "action_repr": str(controller_action_repr),

        "controller_action_index": int(controller_action_index),
        "controller_canonical_action_index": int(controller_canonical_action_index),
        "controller_alias_action_indices": [int(x) for x in controller_alias_action_indices],
        "controller_action_repr": str(controller_action_repr),
        "adversary_action_index": int(adversary_action_index),
        "adversary_action_repr": str(adversary_action_repr),

        "player_acted": "controller",
        "player_to_act_next": str(next_player),

        # Reward/discount are the controller edge from parent -> intermediate.
        # The adversary edge changes the future state but normally has no
        # immediate service-time reward.
        "reward": float(reward),
        "discount": float(discount),
        "controller_reward": float(reward),
        "controller_discount": float(discount),
        "q_no_bootstrap": float(q_no_bootstrap),

        "num_valid_actions": int(num_controller_valid_actions),
        "num_canonical_actions": int(num_controller_canonical_actions),
        "num_controller_valid_actions": int(num_controller_valid_actions),
        "num_controller_canonical_actions": int(num_controller_canonical_actions),
        "num_adversary_valid_actions": int(num_adversary_valid_actions),

        "parent_snapshot_hash": _snapshot_hash(parent_snapshot),
        "parent_stats_hash": _stats_hash(parent_stats),
        "intermediate_snapshot_hash": _snapshot_hash(intermediate_snapshot),
        "intermediate_stats_hash": _stats_hash(intermediate_stats),
    ...
    row.update(_state_summary("intermediate", intermediate_state, cost=intermediate_cost))
...
def generate_child_states(
    *,
    ...
    """Generate cached controller-then-adversary transitions for one root.

    Each canonical controller action first creates an intermediate adversary-to-act
    state. Then every valid adversary action is applied from a fresh copy of that
    intermediate state. Bellman backups must group rows by controller action,
    take min over adversary responses, then max over controller actions.
    """

    parent_player = str(root_record.get("root_player", "controller"))
    if parent_player != "controller":
        raise ValueError(
            "rootChildGenerationAdv currently supports controller roots only; "
    ...
    for canonical_action_index in canonical_indices:
        controller_action = actions_by_index[canonical_action_index]
        if controller_action is None:
            continue

        intermediate_state = mcts._scratch_restore(decision_snapshot, decision_stats)
        intermediate_state = mcts._env.apply_controller_action_only(
            intermediate_state,
            controller_action,
            inplace=True,
            fast_forward=False,
        ...
        q, reward, discount, _bootstrap, intermediate_cost, intermediate_time = mcts._compose_q_from_state(
            leaf_state=intermediate_state,
        ...
        intermediate_snapshot, intermediate_stats = _snapshot_state_and_stats(mcts, intermediate_state)
        alias_action_indices = [
            int(x) for x in canon_to_aliases.get(int(canonical_action_index), [canonical_action_index])
        ]

        adversary_actions_by_index, adversary_mask_t = mcts._actions_and_mask(
            intermediate_state,
            "adversary",
            forbidden_stop_ids=None,
        )
        adversary_valid_mask = [bool(x) for x in adversary_mask_t.tolist()]
        adversary_valid_indices = [
            int(i)
            for i, ok in enumerate(adversary_valid_mask)
            if ok and adversary_actions_by_index[i] is not None
        ]
        if not adversary_valid_indices:
            adversary_valid_indices = [-1]

        for adversary_action_index in adversary_valid_indices:
            if int(adversary_action_index) < 0:
                child_state = intermediate_state
                child_snapshot, child_stats = intermediate_snapshot, intermediate_stats
                child_cost = float(intermediate_cost)
                child_time = float(intermediate_time)
                adversary_action_repr = ""
            else:
                adversary_action = adversary_actions_by_index[adversary_action_index]
                if adversary_action is None:
                    continue
                child_state = mcts._scratch_restore(intermediate_snapshot, intermediate_stats)
                child_state = mcts._env.apply_adversary_action_only(
                    child_state,
                    adversary_action,
                    inplace=True,
                )
                ...
                adversary_action_repr = repr(adversary_action)
            ...
            transition_row = build_transition_trace_row(
                ...
                controller_action_index=int(canonical_action_index),
                controller_canonical_action_index=int(canonical_action_index),
                controller_alias_action_indices=alias_action_indices,
                controller_action_repr=repr(controller_action),
                adversary_action_index=int(adversary_action_index),
                adversary_action_repr=adversary_action_repr,
                parent_cost=parent_cost,
                ...
                num_controller_valid_actions=len(valid_indices),
                num_controller_canonical_actions=len(canonical_indices),
                num_adversary_valid_actions=(
                    0 if adversary_valid_indices == [-1] else len(adversary_valid_indices)
                ),
            )

            key = (int(canonical_action_index), int(adversary_action_index))
            canonical_rows[key] = {
                "schema_version": SCHEMA_VERSION,
                "transition_type": TRANSITION_TYPE,
                "parent_state_id": int(parent_state_id),
                "parent_root_id": int(root_record.get("root_id", -1)),
                # Backward-compatible fields identify the controller action.
                ...
                "action_repr": repr(controller_action),
                "controller_action_index": int(canonical_action_index),
                "controller_canonical_action_index": int(canonical_action_index),
                "controller_alias_action_indices": alias_action_indices,
                "controller_action_repr": repr(controller_action),
                ...
                "controller_reward": float(reward),
                "controller_discount": float(discount),
                "intermediate_cost": float(intermediate_cost),
                "intermediate_time": float(intermediate_time),
                "intermediate_simulator_snapshot": intermediate_snapshot,
                "intermediate_stats": intermediate_stats,
    ...
    for action_index in valid_indices:
        ...
        for key in sorted(k for k in canonical_rows if int(k[0]) == int(canonical_action_index)):
            base = canonical_rows.get(key)
            if base is None:
                continue
            alias_row = dict(base)
            ...
            alias_row["controller_action_index"] = int(action_index)
            alias_row["controller_canonical_action_index"] = int(canonical_action_index)
            alias_row["controller_action_repr"] = repr(action)
...
def store_child_states(
    ...
    *,
    ...
    canonical_count = sum(
        ...
        if int(row.get("controller_action_index", row["action_index"]))
        == int(row.get("controller_canonical_action_index", row["canonical_action_index"]))
    )
...
def assert_transition_rows_match(
    ...
    *,
    ...
    exact_keys = [
        ...
        "controller_action_repr",
        "adversary_action_index",
        "adversary_action_repr",
        "player_acted",
        "player_to_act_next",
        "parent_snapshot_hash",
        "intermediate_snapshot_hash",
        "child_snapshot_hash",
    ]

    float_keys = [
        "parent_sim_time",
        "intermediate_sim_time",
        "child_sim_time",
        "parent_cost",
        "intermediate_cost",
        "child_cost",
        ...
        "intermediate_total_lateness",
        "child_total_lateness",
        "parent_total_cost",
        "intermediate_total_cost",
        "child_total_cost",
    ]
    ...
    list_keys = [
        ...
        "intermediate_violated_request_ids",
        "child_violated_request_ids",
        "alias_action_indices",
        "controller_alias_action_indices",
    ]
    ...
    *,
    ...
    for cached in child_states:
        ...
        for row in regenerated:
            if (
                int(row.get("controller_action_index", row["action_index"]))
                == int(cached.get("controller_action_index", cached["action_index"]))
                and int(row.get("adversary_action_index", -1))
                == int(cached.get("adversary_action_index", -1))
            ):
                match = row
                break
        ...
        if match is None:
            raise AssertionError(
                ...
                f"controller_action_index={cached.get('controller_action_index', cached['action_index'])}, "
                f"adversary_action_index={cached.get('adversary_action_index', -1)} "
                f"for parent_state_id={parent_state_id}"
            )

        for key in ("reward", "discount", "intermediate_cost", "intermediate_time", "child_cost", "child_time"):
            a = float(cached[key])
            b = float(match[key])
            if abs(a - b) > float(atol):
                raise AssertionError(
