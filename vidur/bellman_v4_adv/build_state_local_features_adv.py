...
Schema (in order):
  ...
  - 23 globals (skip the 6 forbidden cols):
      ...
      g_delta_next_adv_tick_norm, g_delta_since_last_adv_launch_norm
...
D_DECODE_PER_SLOT = 4
D_GLOBAL = 23  # 19 base + 2 EDF-vs-decode-batch globals + 2 adversary timing globals
...
def extract_features_one_record(record: dict) -> np.ndarray:
    ...
    # Gather only requests that are still in the game-state active set.
    # Snapshot request_states may retain finalized/historical records for feature
    # parity and debugging, so an empty active_request_ids set means there are no
    # in-system requests, not that every snapshot request should be treated active.
    ...
    globals_vec = [
        ...
        _norm01(delta_next_adv_tick, F_ADV_TICK_SEC_DEN),
        _norm01(delta_since_last_adv_launch, F_LAUNCH_EWMA_WINDOW_SEC),
    ]
