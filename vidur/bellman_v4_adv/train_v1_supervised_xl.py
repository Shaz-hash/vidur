"""
XL capacity sweep for V1 supervised on v4 features.

Tries three hgb_sq configurations near the 120k param ceiling, holding everything
else (loss, learning rate, l2, sample weighting) fixed to the previous champion.

n_params estimate (3 * sum_of_leaves):
  31 leaf x 1280 iter = 31*1280*3  = 119,040
  63 leaf x  630 iter = 63*630*3   = 119,070
  47 leaf x  850 iter = 47*850*3   = 119,850
"""
...
def main() -> None:
    ...
    parser.add_argument("--tail-alpha", type=float, default=4.0)
    ...
    print(f"[xl] X={X.shape} y={y.shape}")
    ...
    X_eval = X[eval_idx]
    y_eval = y[eval_idx]
    print(f"[xl] train n={len(train_idx)} eval n={len(eval_idx)}")
    ...
    for name, est in models.items():
        ...
        print(f"[xl] {name}: fit elapsed={fit_elapsed:.1f}s n_params={n_params:,}")
        ...
        print(f"[xl] {name} TRAIN {m_train}")
        print(f"[xl] {name} EVAL  {m_eval}")
    ...
    for s in summary:
        print(
            f"  {s['name']:30s} params={s['n_params_estimate']:>8,d} "
