...
if [[ "$PHYSICAL_GPU" != "2" ]]; then
    echo "the initial native-DNN controller integration is restricted to physical GPU 2" >&2
    exit 2
fi
...
export VIDUR_VLLM_GV3_STATE_PLANNER="vidur_vllm_real_testing.native_dnn_mcts_planner:PromotedNativeDNNMCTSPlanner"
